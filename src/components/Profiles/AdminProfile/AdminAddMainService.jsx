...
const AdminAddMainService = () => {
  ...
  const [inputStartsFromThird, setInputStartsFromThird] = useState([]);
  const [inputDiscountThird, setInputDiscountThird] = useState([]);
  const [inputDetailsThird, setInputDetailsThird] = useState([]);
  const [thirdLayerArray, setThirdLayerArray] = useState([]);
  ...
  const [imageThirdLayer, setImageThirdLayer] = useState();

  const [subCategoryId, setSubCategoryId] = useState();
  const [thirdheading, setThirdHeading] = useState("");
  const [base64Third, setBase64Third] = useState("");

  const [modalShow, setModalShow] = useState(false);
  const [modalShowDetails, setModalShowDetails] = useState(false);
  const [modalShowThirdDetails, setModalShowThirdDetails] = useState(false);
  const [modalShowThirdDiscount, setModalShowThirdDiscount] = useState(false);
...
const showConfirmboxSecondLayer = (id) => {
  ...
  const handleThirdDetailsModal = () => {   
    //setModalShowDetails(true) 
    try {
      fetchThirdLayerDetails();
      setModalShowThirdDetails(true)
    } catch (error) {
      setModalShowThirdDetails(true)
    }
    //fetchSubCategoryDetails().then(() => setModalShowDetails(true));
  ...
  const fetchThirdLayerDetails = async () => {
    //serviceId // here is third layer id
    var newArray = thirdLayerArray.filter(function (el) {
      return el.id == serviceId
    });
    setInputDetailsThird(newArray[0].service_description);
    setInputDiscountThird(newArray[0].discount);
    setInputStartsFromThird(newArray[0].starts_from);

    //console.log("this is fetch third details id" + serviceId);
...
/*delete functions END*/
  ...
  const fetchItemService = async (id) => {
    ...
    setThirdLayerArray(itemCatagory);
...
//image2 is cover
  ...
  const uploadonChangeImageThirdLayer=(e)=> {
    let files = e.target.files || e.dataTransfer.files;
    if (!files.length)
          return;
          createImageThirdLayer(files[0]);
  }
  const createImageThirdLayer=(file)=> {
    let reader = new FileReader();
    reader.onload = (e) => {
      setImageThirdLayer(e.target.result);
    };

    reader.readAsDataURL(file);
  }
  const onFormSubmitThird=()=>{
    if(inputDetailsThird!==""){
      fileUploadThirdLayer(imageThirdLayer);
    }else{
  ...
  const onFormSubmitThirdDiscount=async()=>{
    ...
    if(item.status == "success"){
      setModalShowThirdDiscount(false)

      confirmAlert({
        title: 'Discount Set!',
        message: 'Do you wish to change anything you just set?',
        buttons: [
          {
            label: 'Yes',
            onClick: () => setModalShowThirdDiscount(true)
          },
          {
            label: 'No',
            onClick: () => {
              setModalShowThirdDiscount(false)
              setInputDiscountThird("")                
            }
          }
        ]
      });
  ...
  //form submit 3rd Layer modal Start

  const fileUploadThirdLayer=(image)=>{
    console.log("file upload hit")
    const success = () =>{
      setModalShowThirdDetails(false)
        confirmAlert({
          title: 'Details Updated!',
          message: 'Do you wish to change anything you just updated?',
          buttons: [
            {
              label: 'Yes',
              onClick: () => setModalShowThirdDetails(true)
            },
            {
              label: 'No',
              onClick: () => {
                setModalShowThirdDetails(false)
                setImageThirdLayer("")
                setInputDetailsThird("")
                setInputStartsFromThird("")                
              }
            }
          ]
        });
      ...
      post(url,  
        {
          serviceid: serviceId,          
          iconfile: image,          
          description: inputDetailsThird,
          starts_from: inputStartsFromThird
  ...
  const fileUploadThird=()=>{
    ...
    try {
      fetch(link).then(response => success() )
    } catch (error) {
      alert("error!")
    }
    
  }
  const changeImageThird=(e)=>{
    const file = e.target.files[0];

    var reader = new FileReader();
   reader.readAsDataURL(file);
   reader.onload = function () {
     setBase64Third(reader.result);
  ...
  //form submit 3rd Layer modal END
  ...
  return (
    <div>
      ...
      <Modal
        size="lg"
        show={modalShowThirdDiscount}
        aria-labelledby="contained-modal-title-vcenter"
        centered
        onHide={() => setModalShowThirdDiscount(false)}

      >
        <Modal.Header>
          <h1 className="text-success" style={{ textAlign: "center" }}>
            Set Discount to your Service
              </h1>
        </Modal.Header>
        <Modal.Body>
          <div>           
            ...
            <div style={{ marginTop: "20px" }}>
              ...
              <input
                ...
                name=""
                id=""
                value={inputDiscountThird}
                onChange={(event) => setInputDiscountThird(event.target.value)}
        ...
        <Modal.Footer>
          <Button className="container btn-success" onClick={() => onFormSubmitThirdDiscount()}>
            Done
            </Button>
        </Modal.Footer>
        </Modal>
      <Modal
        size="lg"
        show={modalShowThirdDetails}
        aria-labelledby="contained-modal-title-vcenter"
        centered
        onHide={() => setModalShowThirdDetails(false)}

      >
        <Modal.Header>
          <h1 className="text-success" style={{ textAlign: "center" }}>
            Adding Details to your Service
              </h1>
        </Modal.Header>
        <Modal.Body>
          <div>
              <div style={{ marginTop: "20px" }}>
                ...
                <label style={{marginTop:"0"}}>Cover File</label>
                  ...
                  onChange={uploadonChangeImageThirdLayer} />
              ...
              <input
                ...
                name=""
                id=""
                value={inputDetailsThird}
                onChange={(event) => setInputDetailsThird(event.target.value)}
                placeholder="Add Details"
                required
              />
            </div>
            ...
            <div style={{ marginTop: "20px" }}>
              ...
              <input
                ...
                name=""
                id=""
                value={inputStartsFromThird}
                onChange={(event) => setInputStartsFromThird(event.target.value)}
                placeholder="Add Details"
                required
              />
            </div>
        ...
        <Modal.Footer>
          <Button className="container btn-success" onClick={() => onFormSubmitThird()}>
            Done
            </Button>
        </Modal.Footer>
        </Modal>
              ...
              <div className="bg-white p-5" style={{ marginTop: "30px" }}>
                ...
                <div className="row justify-content-center">
                  ...
                  <div className="col-md-2">
                    <button
                      className="btn btn-success"
                      onClick={() => handleThirdDetailsModal()}
                      style={{ marginTop: "25px", width:"100%" }}>
                      Add Details
                    </button>
