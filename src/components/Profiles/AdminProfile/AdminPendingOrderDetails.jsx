...
function AdminPendingOrderDetails({ match }) {
  ...
  const fetchItem = async () => {
    setVendors([]);
    const link = "https://kentradigital.com/api/pendingServices?orderid=" + match.params.id;
                      ...
                      <div className="cart-item" style={{ backgroundColor: clicked == item.id ? "#33669A" : "#33669A20", color: clicked == item.id ? "white" : "black" }}>
                        <Row style={{ cursor: "pointer" }}
                          ...
                          <h4
                            ...
                            key={item.id}
                          >
                           {item.service} <br /><span style={{fontSize:"14px"}}>{item.service_name} <br/><br/> Price: {item.price} <br/>Area: {item.area}</span>
